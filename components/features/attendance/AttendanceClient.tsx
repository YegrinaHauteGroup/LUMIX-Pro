    ...
    <div className="flex-1 p-5 space-y-4 overflow-auto">
      ...
      <div className="flex items-center gap-2">
        <input type="date" value={checkDate} onChange={(e) => setCheckDate(e.target.value)}
          className="bg-[#0e0e0e] border border-[#1e1e1e] px-3 text-[12px] text-[#e8e8e8] focus:outline-none focus:border-[#333333] h-8 rounded-sm" />
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="bg-[#0e0e0e] border border-[#1e1e1e] px-4 py-3">
            <p className="text-[10px] text-[#444444] uppercase tracking-widest mb-1">{s.label}</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-[#444444]">{filtered.length}명</span>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          {loadingData ? (
            <p className="text-center text-[12px] text-[#333333] py-10">불러오는 중...</p>
          ) : filtered.length === 0 ? (
            <p className="text-center text-[12px] text-[#333333] py-10">재원 아동이 없습니다</p>
          ) : (
            <div>
              {filtered.map((child) => {
                const status = attendance[child.id]
                return (
                  <div key={child.id} className="flex items-center gap-4 px-5 py-3 border-b border-[#111111] hover:bg-[#0c0c0c] transition-colors last:border-0">
                    <div className="w-6 h-6 bg-[#141414] border border-[#1e1e1e] flex items-center justify-center shrink-0">
                      <span className="text-[10px] text-[#555555]">{child.name[0]}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] text-[#cccccc] font-medium">{child.name}</p>
                      <p className="text-[10px] text-[#444444]">{(child.classes as Class | null)?.name ?? '반 미배정'}</p>
                    </div>
                      ...
                      {(Object.keys(ATTENDANCE_STATUS_LABELS) as AttStatus[]).map((s) => (
                        <button key={s} onClick={() => handleStatus(child.id, s)} disabled={saving === child.id}
                          className={`flex items-center gap-1 px-2.5 py-1 text-[11px] font-medium transition-colors rounded-sm ${
                            status === s ? ATTENDANCE_STATUS_COLORS[s] : 'text-[#333333] hover:text-[#666666] hover:bg-[#141414]'
                          }`}>
