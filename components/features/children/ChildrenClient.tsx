    ...
    <div className="flex-1 p-5 space-y-4 overflow-auto">
      ...
      <div className="flex items-center gap-2">
        <div className="relative">
          <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#444444]" />
          <input
            type="text"
            placeholder="이름 검색"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-52 bg-[#0e0e0e] border border-[#1e1e1e] pl-8 pr-3 py-1.5 text-[12px] text-[#e8e8e8] placeholder-[#333333] focus:outline-none focus:border-[#333333] h-8 rounded-sm"
          />
        </div>
        <select
          value={filterStatus}
          onChange={(e) => setFilterStatus(e.target.value as typeof filterStatus)}
          className="bg-[#0e0e0e] border border-[#1e1e1e] px-3 py-1.5 text-[12px] text-[#888888] focus:outline-none focus:border-[#333333] h-8 rounded-sm cursor-pointer"
        >
        ...
        <span className="text-[11px] text-[#444444]">{filtered.length}명</span>
            ...
            <thead>
              <tr className="border-b border-[#1a1a1a]">
                {['이름', '성별', '나이', '반', '보호자', '연락처', '상태', ''].map((h) => (
                  <th key={h} className="text-left text-[10px] text-[#444444] font-medium uppercase tracking-widest px-4 py-3 first:pl-5">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-16">
                    <Users size={28} className="mx-auto mb-2 text-[#1e1e1e]" />
                    <p className="text-[12px] text-[#333333]">등록된 아동이 없습니다</p>
                  </td>
                </tr>
              ) : filtered.map((child) => (
                <tr key={child.id} className="border-b border-[#111111] hover:bg-[#0c0c0c] transition-colors">
                  <td className="px-4 py-3 first:pl-5">
                    <Link href={`/children/${child.id}`} className="flex items-center gap-2.5 group">
                      <div className="w-6 h-6 bg-[#141414] border border-[#222222] flex items-center justify-center shrink-0">
                        <span className="text-[10px] text-[#888888]">{child.name[0]}</span>
                      </div>
                      <span className="text-[12px] font-medium text-[#cccccc] group-hover:text-[#e8e8e8] transition-colors">
                        {child.name}
                      </span>
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-[#666666]">{GENDER_LABELS[child.gender]}</td>
                  ...
                  <td className="px-4 py-3 text-[12px] text-[#666666]">{child.guardian_name ?? '—'}</td>
                  <td className="px-4 py-3 text-[12px] text-[#666666]">{child.guardian_phone ?? '—'}</td>
                  <td className="px-4 py-3">
                    <Badge className={CHILD_STATUS_COLORS[child.status]}>{CHILD_STATUS_LABELS[child.status]}</Badge>
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => handleDelete(child.id)} className="text-[#2a2a2a] hover:text-[#ef4444] transition-colors p-1">
                      <Trash2 size={13} />
                    </button>
                  </td>
                </tr>
