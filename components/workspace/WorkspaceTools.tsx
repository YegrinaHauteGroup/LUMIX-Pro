...
function Stopwatch() {
  ...
  return (
    <div className="space-y-1.5">
      <div className="text-center text-[26px] font-data tabular-nums text-ink py-1.5 leading-none">{fmtSw(sw.elapsed)}</div>
      <div className="grid grid-cols-3 gap-1.5">
        <button onClick={() => clock.swToggle()} className={`h-8 rounded-[3px] text-[11px] font-medium inline-flex items-center justify-center gap-1 ${sw.running ? 'bg-warn-soft text-warn' : 'bg-accent text-white hover:bg-accent-hover'}`}>
          {sw.running ? <><Pause size={12} /> 정지</> : <><Play size={12} /> 시작</>}
        </button>
        <button onClick={() => clock.swLap()} disabled={!sw.running} className="h-8 rounded-[3px] text-[11px] font-medium border border-line text-ink-soft hover:bg-fill disabled:opacity-40">랩</button>
        <button onClick={() => clock.swReset()} className="h-8 rounded-[3px] text-[11px] font-medium border border-line text-ink-soft hover:bg-fill inline-flex items-center justify-center gap-1"><RotateCcw size={12} /> 초기화</button>
      </div>
      {sw.laps.length > 0 && (
        <div className="max-h-24 overflow-y-auto border border-line rounded-[3px] divide-y divide-line">
          {sw.laps.map((l, i) => (
            <div key={i} className="flex items-center justify-between px-2 py-1 text-[10.5px]">
              <span className="text-ink-faint">랩 {sw.laps.length - i}</span>
              <span className="font-data tabular-nums text-ink-soft">{fmtSw(l)}</span>
            </div>
          ))}
        </div>
      )}
...
const fmtTm = (t: number) => { const s = Math.ceil(t / 1000); return `${String(Math.floor(s / 60)).padStart(2, '0')}:${String(s % 60).padStart(2, '0')}` }

// ── Timer ───────────────────────────────────────────────────────────────────
function Timer() {
  const { tm } = useClock()
  const [input, setInput] = useState({ m: 5, s: 0 })
  const idle = !tm.running && tm.remaining <= 0
  return (
    <div className="space-y-1.5">
      {idle ? (
        ...
        <div className={`text-center text-[30px] font-data tabular-nums py-1.5 leading-none ${tm.done ? 'text-danger animate-pulse' : 'text-ink'}`}>{fmtTm(tm.remaining)}</div>
      )}
      <div className="grid grid-cols-2 gap-1.5">
        <button onClick={() => (tm.running ? clock.tmPause() : clock.tmStart((input.m * 60 + input.s) * 1000))} className={`h-8 rounded-[3px] text-[11px] font-medium inline-flex items-center justify-center gap-1 ${tm.running ? 'bg-warn-soft text-warn' : 'bg-accent text-white hover:bg-accent-hover'}`}>
          {tm.running ? <><Pause size={12} /> 일시정지</> : <><Play size={12} /> 시작</>}
        </button>
        <button onClick={() => clock.tmReset()} className="h-8 rounded-[3px] text-[11px] font-medium border border-line text-ink-soft hover:bg-fill inline-flex items-center justify-center gap-1"><RotateCcw size={12} /> 초기화</button>
      </div>
    </div>
  )
}
...
export function WorkspaceTools() {
  ...
  return (
    <div className="border border-line rounded-[3px] bg-surface overflow-hidden">
      ...
      {/* All three stay mounted; inactive ones are hidden so stopwatch/timer keep
          running and never reset when switching tabs. */}
      <div className="p-2">
        <div className={tab === 'calc' ? '' : 'hidden'}><Calculator /></div>
        <div className={tab === 'watch' ? '' : 'hidden'}><Stopwatch /></div>
        <div className={tab === 'timer' ? '' : 'hidden'}><Timer /></div>
      </div>
    </div>
  )
}
